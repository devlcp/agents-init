...
## 🎯 Protocol
1. **Language Check:** "In which language do you want to conduct this setup?"
2. **Discovery Phase:** Perform a quick scan of the root directory. Identify key files (package.json, go.mod, Dockerfile, src/, etc.).
3. **Analysis Report:** Briefly summarize what you've found and propose a technical configuration based on the stack.
4. **Refinement Interview:** Ask targeted questions only if the discovery is ambiguous or to confirm specific preferences.
5. **Drafting:** Generate the AGENTS.md in ENGLISH based on the discovered stack and refined answers.
...
### 0. Language Check
"In which language do you want to conduct this setup?"
*Note: Translate all subsequent questions into the chosen language.*
...
3. **Refactor (R):** Populate the `## 🧩 Skills Inventory` based on the discovered stack.
